...
import pymupdf  # pip install --upgrade pymupdf
...
def process_pdf(pdf_path: Path):
    try:
        ...
        if fio and iin:
            ...
            print(f"OK: {pdf_path.name} -> {final_path.name}")
        else:
            print(f"SKIP: {pdf_path.name} (ФИО/ИИН не найдены)")
...
if not base.exists():
    print(f"ERROR: Папка не найдена: {base}")
    raise SystemExit(1)
