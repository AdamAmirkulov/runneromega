...
### `RENAME_ROOT` — папка для скриптов переименования

В `scripts/config.py` **на сервере** должна быть строка:
```python
RENAME_ROOT = r'<путь, каким его видит СЕРВЕР>'   # напр. r'\\SRVAPP\work folder'
```
Внутри неё лежат подпапки-партии. В веб-форме скриптов переименования
(Vivus / ММ / Lime / Solva / Надписи / Отмены надписей / Госпошлины /
MSG→PDF) сотрудник **выбирает подпапку из выпадающего списка** — путь
руками больше не вводится (у сотрудников шара смонтирована по-разному,
а скрипт всё равно выполняется на сервере). `app.py` читает эту строку
из `config.py` без импорта модуля; можно вместо неё задать переменную
окружения `RENAME_ROOT`. Если не задано — в форме остаётся поле ручного
ввода полного пути.
